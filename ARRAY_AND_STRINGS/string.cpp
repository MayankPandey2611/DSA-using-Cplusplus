...
string reverseWords(string input)
{
    ...
    {
        // If we encounter a space and we have a word, we add it to the result
        if (input[i] == ' ')
        {
            if (!word.empty())
            {
                ...
                {
                    reversedString += ' '; // Add space before the next word
                }
                reversedString += word;
    ...
    if (!word.empty())
    {
        if (!reversedString.empty())
        {
            reversedString += ' '; // Add space before the last word
        }
        reversedString += word;
    }
